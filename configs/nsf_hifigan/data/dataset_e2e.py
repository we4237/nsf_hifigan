...
class MelDataset(torch.utils.data.Dataset):
    ...
    def get_item(self, index: int):
        ...
        diffsinger = np.load(npy,allow_pickle=True)[0]
        wav = audio_wav.cpu().numpy()
        if len(diffsinger)-len(wav) > 0:
            r_pad = len(diffsinger)-len(wav)
            waveform = np.pad(wav, (0, r_pad), mode='constant', constant_values=0.0)
        ...
        return {
            ...
            "mel": audio_mel,
            "pitch": audio_pitch,
            "wav_padded":waveform.unsqueeze(0),
        }
